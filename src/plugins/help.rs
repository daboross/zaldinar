...
fn help(event: &interface::CommandEvent) {
    ...
    event.client.send_message(event.channel, format!("Use a command with `{}command_name` or `{}, command_name`", event.client.config.command_prefix, event.client.state.read().nick).as_slice());
}
